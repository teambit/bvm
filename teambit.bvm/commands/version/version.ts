...
export class VersionCmd implements CommandModule {
  ...
  async handler(args) {
    ...
    const latestBvmRemoteVersion = args.includeRemote ?  await getBvmRemoteVersion() : undefined;
    ...
    const output = formatOutput({currentBvmVersion,
      latestBvmRemoteVersion, currentVersion,
...
function formatOutput(versions: VersionsResult): string {
  const currentBvmVersionOutput = versions.currentBvmVersion ? `current (used) bvm version: ${chalk.green(versions.currentBvmVersion)}` : undefined;
  const latestRemoteBvmOutput = versions.latestBvmRemoteVersion ? `latest available bvm version: ${chalk.green(versions.latestBvmRemoteVersion)}` : undefined;

  const currentVersionOutput = versions.currentVersion ? `current (used) bit version: ${chalk.green(versions.currentVersion)}` : undefined;
  const latestInstalled = versions.latestInstalledVersion ? `latest installed bit version: ${chalk.green(versions.latestInstalledVersion)}` : undefined;
  const latestRemote = versions.latestRemoteVersion ? `latest available bit version: ${chalk.green(versions.latestRemoteVersion)}` : undefined;
  ...
  const outputs = [currentBvmVersionOutput, 
    latestRemoteBvmOutput, currentVersionOutput,
    latestInstalled,
    latestRemote, '\n', newerBvmOutput, newerBitOutput].filter(output => output);
  return outputs.join('\n');
}

function getNewerBvmAvailableOutput(currentBvmVersion?: string, latestBvmRemoteVersion?: string): string | undefined {
  ...
  if (semver.gt(latestBvmRemoteVersion, currentBvmVersion)){
    ...
    return `new version of ${chalk.cyan('bvm')} is available, upgrade your ${chalk.cyan('bvm')} by running "${npmCommand}" or "${yarnCommand}"`
  }
}

function getNewerBitAvailableOutput(currentVersion? : string,
  latestInstalledVersion?: string,
  latestRemoteVersion?: string): string {
  if (!currentVersion) {
    return undefined;
  }
  ...
  if (semver.gt(latestInstalledVersion, currentVersion) || (latestRemoteVersion && semver.gt(latestRemoteVersion, currentVersion))){
    return `new version of ${chalk.cyan('bit')} is available, upgrade your ${chalk.cyan('bit')} by running "${chalk.cyan("bvm upgrade")}"`
  }
}
